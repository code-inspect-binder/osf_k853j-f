...
# ESS 2016 (round 9)
#
# Note: first run the code in the "ESS_openness_2018.R" script
#--------------------------------------------------------------------------------------------------------
...
nrow(countries_sort)
length(unique(countries_sort$Country))
...
for (c in countries) {
  ...
  data_c = data9 %>% filter(cntry_full == c) # select only data from a country c 
...
for (df in listofdfs) {
  ...
  # get model summary (without intercept) --> it gives coefs for the second row (first variables after intercept, prob. needed for recovery data only)
...
betalms <- 
  ...
  left_join(data9 %>% dplyr::select(cntry_full, welstate) %>% group_by(cntry_full, welstate) %>% 
              slice(1) %>% 
              ungroup() %>%
...
#---- Plot beta weights for openness (or now conservation = needs for security and certainty)
betalms_ope
